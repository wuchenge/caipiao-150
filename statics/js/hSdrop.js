...
kl+='		<div class="cancel">'+(getCookie('zh_choose')=='e'?'Cancel':'取消')+'</div>'
kl+='		<div class="determine">'+(getCookie('zh_choose')=='e'?'OK':'确定')+'</div>'
kl+='	</div>'
...
kl+='</div>'
kl+='</div>'
$('body').append(kl);var swipersff=new Swiper('.sw-select',{direction:'vertical',autoHeight:true,slidesPerView:3,centeredSlides:true,observer:true,observeParents:true,observeSlideChildren:true,});$('.sw-select .swiper-wrapper>div').click(function(e){swipersff.slideTo($(this).index(),500,false);});$('.select-moob .cancel,.select-moob-cover').click(function(e){$('.select-moob').remove();});$('.select-moob .determine').click(function(e){$(el).children('input').val($('.sw-select .swiper-slide-active').html());
$(el).children('input').attr('value',$('.sw-select .swiper-slide-active').innerText);
$('#select-value').attr('value',$('.sw-select .swiper-slide-active').attr('vid'));
$('#select-value').click();
$('.select-moob').remove();})},}
