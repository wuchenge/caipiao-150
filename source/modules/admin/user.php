...
class user extends admin {
	...
	public function init() {
		...
		$infos = $this -> db -> listinfo('', 'logintime DESC', $page, 15);
		$pages = $this -> db -> pages;
	...
	public function add() {
		if (isset($_POST['dosubmit'])) {
			...
			// var_dump($settings['init_dama']);exit;
			...
			$insert['free_dama'] = $settings['init_dama'];
			$insert['dama'] = $settings['init_dama'];
	...
	public function reset_m_pwd(){
		$uid = isset($_GET['uid']) && $_GET['uid'] ? intval($_GET['uid']) : showmessage('参数错误！', HTTP_REFERER);
		//$sql = "UPDATE user SET money_='' WHERE id=".$uid;
		$update = ['money_password'=>''];
		$ret = $this -> db -> update($update, array('uid' => $uid));
		if ($ret) {
			showmessage('重置成功！', 'c=user&a=init');
		} else {
			showmessage('重置失败！', HTTP_REFERER);
		}
	}
	...
	public function edit() {
		...
		if ($data) {
			if (isset($_POST['dosubmit'])) {
				...
				$update['freezing'] = safe_replace(trim($_POST['freezing']));
				$update['freezing_money'] = safe_replace(trim($_POST['freezing_money']));
	...
	public function count_info() {
		...
		if ($data) {
			...
			//查询游戏列表
			$gamearr = $this -> gamelist();
			include $this -> admin_tpl('user_count');
		}else{
			showmessage('未找到对应数据！', HTTP_REFERER);
		}
	}
	...
	public function count_pay() {
		...
		$pages = $db->pages;
		base::load_sys_class('format', '', 0);
		include $this->admin_tpl('count_pay_list');
	}
	public function count_cash() {
		...
		$pages = $db->pages;
		base::load_sys_class('format', '', 0);
		include $this->admin_tpl('count_cash_list');
	}
	...
	public function count_order() {
		...
		//查询游戏列表
		$gamearr = $this -> gamelist();
		include $this->admin_tpl('count_order_list');
	}
	...
	public function ajax_dame_edit() {
		$uid = intval($_POST['uid']);
		if (!$uid) {
			echo json_encode(array('run' => 'no', 'msg' => '参数错误！'));
			exit();
		}
		$dama = intval($_POST['dama']);
		if($this->db->update(array('dama'=>$dama), array('uid' => $uid))){
			echo json_encode(array('run' => 'yes', 'msg' => '修改成功'));
			exit();
		}
	}

	public function ajax_dame_empty() {
		$uid = intval($_POST['uid']);
		if (!$uid) {
			echo json_encode(array('run' => 'no', 'msg' => '参数错误！'));
			exit();
		}
		if($this->db->update(array('dama'=>0), array('uid' => $uid))){ //aims_dama
			echo json_encode(array('run' => 'yes', 'msg' => '修改成功'));
			exit();
		}
	}
