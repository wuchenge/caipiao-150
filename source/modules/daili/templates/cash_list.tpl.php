...
<div class="subnav">
	...
	<div class="content-menu">
		<a href="<?php echo DAILI_PATH?>&c=cash&a=init" class="on"><em>申请列表</em></a>
	</div>
</div>
<div class="content-t">
	<div id="searchshow">
		<form name="searchform" action="<?php echo DAILI_PATH?>&c=cash&a=search" method="get">
	    ...
	    <tbody>
	    	<?php foreach ($list as $v){ ?>
				<tr id="list_<?php echo $v['id']?>">
					...
					<td align="left">
						<input title="点击可编辑" class="input-text no" type="text" value="<?php echo $v['comment']?>" style="width: 200px;" ignore="ignore" datatype="*1-50" ajaxurl="<?php echo DAILI_PATH?>&c=cash&a=ajax_comment&id=<?php echo $v['id']?>" nullmsg="输入备注" errormsg="备注内容在50字内" />
					</td>
					...
					<td align="center">
						<a href="javascript:;" onclick="showwindow('<?php echo DAILI_PATH?>&c=cash&a=ajax_comment&del=yes&id=<?php echo $v['id']?>', '确定清空备注内容？');">[清空]</a>
						<span id="caozuo_<?php echo $v['id']?>">
							<?php if($v['state'] == 0) {?>
							<!--<a href="javascript:;" onclick="showwindow('<?php echo DAILI_PATH?>&c=cash&a=setstate&state=1&id=<?php echo $v['id']?>', '确定转为《正在处理》吗？', 1);">[处理]</a>-->
							<!--<a href="javascript:;" onclick="showwindow('<?php echo DAILI_PATH?>&c=cash&a=setstate&state=3&id=<?php echo $v['id']?>', '确定拒绝并转为《提现失败》吗？<br/>对应申请金额将自动返还给申请账户！<br/>建议备注失败的原因供用户参考。', 1);">[拒绝]</a>-->
							<!--?php } elseif($v['state'] == 1) {?-->
							<a href="javascript:;" onclick="showwindow('<?php echo DAILI_PATH?>&c=cash&a=setstate&state=2&id=<?php echo $v['id']?>', '确定转为《提现成功》吗？', 1);">[成功]</a>
							<a href="javascript:;" onclick="showwindow('<?php echo DAILI_PATH?>&c=cash&a=setstate&state=3&id=<?php echo $v['id']?>', '确定转为《提现失败》吗？<br/>对应申请金额将自动返还给申请账户！', 1);">[失败]</a>
							<?php }?>
						</span>
					</td>
				</tr>
